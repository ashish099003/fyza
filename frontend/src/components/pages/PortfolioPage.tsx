        ...
        <TabsContent value="debt" className="flex-1 overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 h-full overflow-y-auto">
            {debts.map((debt, index) => {
              ...
              return (
                <Card key={index} className="flex flex-col hover:shadow-lg transition-all duration-300 border-0 shadow-sm rounded-2xl bg-gradient-to-br from-card to-card/50">
                  <CardHeader className="pb-1">
                    <div className="flex items-center justify-between">
                      ...
                      >
                        {debt.priority === 'high' ? 'High' : debt.priority === 'medium' ? 'Medium' : 'Low'}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="flex-1 p-3">
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          <div className="text-xs text-center text-muted-foreground">
                            {debt.paidTenure}Y / {debt.totalTenure}Y
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-border/50">
                        ...
                        <Button size="sm" className="bg-[rgba(76,146,102,1)] hover:bg-primary/90 text-xs rounded-full px-4">
                          Pay Now
                        </Button>
                      </div>
                    </div>
            ...
            {insurancePolicies.map((policy, index) => (
              <Card key={index} className={`hover:shadow-lg transition-all duration-300 border-0 shadow-sm rounded-2xl bg-gradient-to-br from-card to-card/50 ${
                ...
                <CardHeader className="pb-0.5 pt-2 px-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl flex items-center justify-center">
                        <Shield className="h-3 w-3 text-blue-600" />
                      </div>
                      <div>
                        <h3 className="font-semibold text-foreground text-sm">{policy.name}</h3>
                        <p className="text-xs text-muted-foreground">{policy.provider}</p>
                      </div>
                    </div>
                    <Badge 
                      variant={policy.recommendation === 'adequate' ? 'default' : 'secondary'}
                      className={`text-xs rounded-full ${policy.recommendation === 'increase' ? 'bg-amber-100 text-amber-800 border-amber-300 hover:bg-amber-200' : ''}`}
                    >
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="grid grid-cols-3 gap-1">
                      ...
                      <div className="text-center p-1.5 bg-blue-50 rounded-lg border border-blue-100">
                        <div className="text-xs font-bold text-blue-600">₹{policy.premium.toLocaleString()}</div>
                        <div className="text-xs text-muted-foreground">Premium</div>
                      </div>
                      <div className="text-center p-1.5 bg-purple-50 rounded-lg border border-purple-100">
                        <div className="text-xs font-bold text-purple-600">{policy.type}</div>
                    ...
                    <div className="flex items-center justify-between pt-1 border-t border-border/50">
                      {policy.recommendation === 'increase' ? (
                        <>
                          ...
                          <Button size="sm" variant="outline" className="text-xs rounded-full px-2 h-5">
                            Increase
                          </Button>
                        </>
                      ) : (
